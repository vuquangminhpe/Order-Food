...
const MenuItemScreen = ({ route, navigation }: any) => {
  ...
  useEffect(() => {
    ...
    Object.values(selectedOptions).forEach((selections) => {
      selections.forEach((option: { price: number }) => {
  ...
  const handleOptionSelect = (
    ...
    setSelectedOptions((prev) => {
      const option = menuItem.options.find(
        (opt: { title: any }) => opt.title === optionTitle
      );
  ...
  const handleAddToCart = () => {
    ...
    const formattedOptions = Object.entries(selectedOptions)
      .map(([title, items]) => ({
        title,
        items: items.map((item: { name: any; price: any }) => ({
          name: item.name,
          price: item.price,
        })),
      }))
            ...
            {menuItem.options && menuItem.options.length > 0 && (
              <View style={styles.optionsContainer}>
                ...
                {menuItem.options.map(
                  (optionGroup: { title: string | number }, index: any) => (
                    <OptionGroup
                      key={index}
                      optionGroup={optionGroup}
                      selectedItems={selectedOptions[optionGroup.title] || []}
                      onSelect={(item: any, isSelected: any) =>
